...
//   0 ..  31: GPIO1 0..31
//  32 ..  63: GPIO2 0..31
//  64 ..  91: GPIO3 0..27  (4 pins not supportetd)
//  96 .. 127: GPIO4 0..31
// 128 .. 130: GPIO5 0..2

#define GPIO_MAX_PORTS          5U
#define GPIO_MAX_PINS           131U
...
// Pin configuration
static pinConfig_t const PinConfig[GPIO_MAX_PINS] = {
...
// GPIO port Active Flags
static uint32_t GpioActive[GPIO_MAX_PORTS] = {
  0U, 0U, 0U, 0U, 0U
};
...
static int32_t GPIO_PowerControl (ARM_GPIO_Pin_t pin, ARM_POWER_STATE state) {
  ...
  if (pin < GPIO_MAX_PINS) {
    ...
    switch (state) {
      case ARM_POWER_OFF:
        GpioActive[pin_port] &= ~(1U << pin_num);
        ...
        if (GpioActive[pin_port] == 0U) {
          NVIC_DisableIRQ(GPIOIRQn[(pin_port << 1U) + (pin_num >> 4U)]);
        }
        break;
      case ARM_POWER_LOW:
        break;
      case ARM_POWER_FULL:
        if ((GpioActive[pin_port] & (1U << pin_num)) == 0U) {
          GpioActive[pin_port] |= (1U << pin_num);
